...
const TARIFFS = {
  ...
  advanced: { name: 'Продвинутый курс', price: '2990.00', description: 'Продвинутый курс — 11 видеоуроков, бессрочный доступ' },
...
async function connectDB() {
  if (!CONFIG.MONGODB_URI) {
    console.log('⚠️ MONGODB_URI не задан — работаем без БД');
    return;
  }
  try {
    const client = new MongoClient(CONFIG.MONGODB_URI);
    await client.connect();
    db = client.db('coachskill');
...
async function saveUser(userData) {
  ...
  console.log(`💾 Сохранён пользователь: ${userData.email}`);
}
...
// Проверка сервера
app.get('/', (req, res) => {
  res.json({ status: 'ok', message: 'CoachSkill сервер работает!', db: db ? 'connected' : 'disconnected' });
});
  ...
  try {
    ...
    const paymentData = {
      ...
      receipt: {
        ...
        items: [{ description: tariffData.description, quantity: '1.00', amount: { value: tariffData.price, currency: 'RUB' }, vat_code: 1, payment_mode: 'full_payment', payment_subject: 'service' }]
      }
    };
...
<body>
  <div class="card">
    ...
    <div class="note">📧 Проверь почту — там логин и пароль.<br>Если не пришло за 5 минут — проверь папку «Спам».</div>
...
app.get('/api/users/:secret', async (req, res) => {
  ...
  if (!db) return res.json({ count: 0, users: [], note: 'MongoDB не подключена' });
...
connectDB().then(() => {
  app.listen(PORT, () => {
    ...
    console.log(`🗄️ MongoDB: ${db ? 'подключена' : 'не подключена'}`);
  });
});
